...
eth_rpl_re = calculate_rd(current_risk_free, eth_rpl_beta, eth_market_premium)

eth_rpl_wacc = calculate_wacc(rpl_mk, rpl_liabilities, eth_rpl_re, rpl_rd)
...
def show_rocketpoolpage():
    ...
    market_value_df = pd.DataFrame({
        'Metric': ['Enterprise Value', 'EV to Revenue'],
        'Value': [ f"${ev_df['ev_historical'].iloc[-1]:,.2f}", f"{ttm_ev_rev:.2f}"]
    })
