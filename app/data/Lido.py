...
bookval_current = abs(equity.iloc[0]) / ldo_supply

market_to_book = ldo_current_price / bookval_current
...
ldo_s = ldo_market_cap['marketcap'] / ldo_history['price']

ldo_fixed = ldo_s.groupby([pd.Grouper(freq='M')]).mean()
...
quarterly = lidoincome_df['($) >>Total Protocol Income'].resample('Q').sum() 
quarterly_supply = ldo_fixed.resample('Q').mean()

quarterly_stats = quarterly.to_frame('net income').merge(quarterly_supply.to_frame('supply'), left_index=True, right_index=True)

income_statement_data = {
    'Cost of Revenue': ttm_metrics['($) >Cost of Revenue'].iloc[0],
    'Liquidity Expenses': ttm_metrics['($) >Liquidity Expenses'].iloc[0],
    'Operating Expenses': ttm_metrics['($) >Operating Expenses'].iloc[0],
    'Net Revenue': ttm_metrics['($) >Net Revenue'].iloc[0],
    'Net Income': ttm_metrics['($) >>Total Protocol Income'].iloc[0]
}

# Convert the dictionary to a DataFrame
income_statement = pd.DataFrame.from_dict(income_statement_data, orient='index', columns=['Amount'])
...
income_statement = pd.DataFrame.from_dict(income_statement_data, orient='index', columns=['Amount'])
# Clean up the index name, if necessary
income_statement.index.name = 'Item'
