...
monthly_stats = revdf.groupby(['period']).last()
...
quarterly_stats = revdf.groupby(['year', 'quarter']).last().reset_index()
quarterly_stats = monthly_stats.resample('Q').sum()
last_four_quarters = quarterly_stats.iloc[-4:]
ttm_data = monthly_stats.tail(12).sum()
...
historical_supply_quarterly = historical_supply.to_frame('supply').resample('Q').last()
dividend_ps = full_metrics['net_income'].to_frame('net income').resample('Q').sum().merge(historical_supply_quarterly,left_index=True, right_index=True )
