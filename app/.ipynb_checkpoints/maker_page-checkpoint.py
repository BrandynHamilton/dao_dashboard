...
def line_plot_metric(df):
    return st.line_chart(df)

def bar_plot_metric(df):
...
shortened_eth_timeframe = mkr_annual_returns.to_frame('mkr returns').merge(eth_annual_returns.to_frame('eth returns'), left_index=True, right_index=True)
shortened_eth_timeframe = shortened_eth_timeframe.merge(tbilldf_yearly['value'].to_frame('tbill'), left_index=True, right_index=True)
eth_short_yearly_risk_premium = shortened_eth_timeframe['eth returns'] - shortened_eth_timeframe['tbill']
average_eth_short_risk = eth_short_yearly_risk_premium.mean()


short_eth = eth_history[eth_history.index > '2020-10-29']
eth_short_cagr = calculate_historical_returns(short_eth)
eth_short_cumulative_risk_premium = eth_short_cagr - current_risk_free
...
mkr_dpi_long_coste = calculate_rd(current_risk_free, dpi_mkr_beta, dpi_cumulative_risk_premium)
mkr_dpi_short_coste = calculate_rd(current_risk_free, dpi_mkr_beta, dpi_market_premium)
mkr_eth_long_coste = calculate_rd(current_risk_free, eth_mkr_beta, eth_cumulative_risk_premium)
mkr_eth_short_coste = calculate_rd(current_risk_free, eth_mkr_beta, eth_market_premium)
    
mkr_dpi_long_wacc = calculate_wacc(mkr_mk, mkr_liabilities, mkr_dpi_long_coste, mkr_rd)
mkr_dpi_short_wacc = calculate_wacc(mkr_mk, mkr_liabilities, mkr_dpi_short_coste, mkr_rd)
mkr_eth_long_wacc = calculate_wacc(mkr_mk, mkr_liabilities, mkr_eth_long_coste, mkr_rd)
mkr_eth_short_wacc = calculate_wacc(mkr_mk, mkr_liabilities, mkr_eth_short_coste, mkr_rd)
