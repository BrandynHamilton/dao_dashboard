...
ldo_dpi_long_coste = calculate_rd(current_risk_free, dpi_ldo_beta, dpi_cumulative_risk_premium)
ldo_dpi_short_coste = calculate_rd(current_risk_free, dpi_ldo_beta, dpi_market_premium)
ldo_eth_long_coste = calculate_rd(current_risk_free, eth_ldo_beta, eth_cumulative_risk_premium)
ldo_eth_short_coste = calculate_rd(current_risk_free, eth_ldo_beta, eth_market_premium)
    
ldo_dpi_long_wacc = calculate_wacc(ldo_mk, ldo_liabilities, ldo_dpi_long_coste, ldo_rd)
ldo_dpi_short_wacc = calculate_wacc(ldo_mk, ldo_liabilities, ldo_dpi_short_coste, ldo_rd)
ldo_eth_long_wacc = calculate_wacc(ldo_mk, ldo_liabilities, ldo_eth_long_coste, ldo_rd)
ldo_eth_short_wacc = calculate_wacc(ldo_mk, ldo_liabilities, ldo_eth_short_coste, ldo_rd)
...
def show_lidopage():
    ...
    market_value_df = pd.DataFrame({
        'Metric': ['Earnings per Share', 'Price to Earnings', 'Book Value', 'Market to Book', 'Enterprise Value', 'EV to Revenue'],
        'Value': [f"{eps:.2f}", f"{price_to_earnings:.2f}", f"{bookval_current:.2f}", f"{market_to_book:.2f}", f"${enterprise_value:,.2f}", f"{ev_to_rev.iloc[0]:.2f}"]
    })
