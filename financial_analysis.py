...
def show_industry_metrics():
    ...
    fig2.add_trace(go.Scatter(x=monthly_metrics.index, y=monthly_metrics['ev_to_rev_truncated'], mode='lines', name='MakerDao EV to Revenue', line=dict(color='red')))
    fig2.add_trace(go.Scatter(x=lido_metrics.index, y=lido_metrics['ev_to_rev'].iloc[:-1], mode='lines', name='LidoDao EV to Revenue', line=dict(color='blue')))
    ...
    fig3.add_trace(go.Scatter(x=monthly_metrics.index, y=monthly_metrics['historical_ev'], mode='lines', name='MakerDao EV', line=dict(color='red')))
    fig3.add_trace(go.Scatter(x=lido_metrics.index, y=lido_metrics['historical_ev'].iloc[:-1], mode='lines', name='LidoDao EV', line=dict(color='blue')))
    fig3.add_trace(go.Scatter(x=ev_df.index, y=ev_df['ev_historical'], mode='lines', name='Rocketpool EV', line=dict(color='yellow')))
